...
class OptionCriticNetwork(nn.Module):
    ...
    def get_action_dist(self, state, option):
        logits = state.detach() @ self.options_W[option] + self.options_b[option]
...
class Agent:
    ...
    def save_models(self):
        print('... saving models ...')
        self.option_critic.save_checkpoint()
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for t in range(len(reward_arr)-1):
                ...
                advantage[t] = a_t
            advantage = T.tensor(advantage).to(self.option_critic.device)

            values = T.tensor(values).to(self.option_critic.device)
            for batch in batches:
                obs = T.tensor(obs_arr[batch], dtype=T.float).to(self.option_critic.device)
                states = self.option_critic.get_state(obs)
