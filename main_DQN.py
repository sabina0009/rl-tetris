...
def run_worker(process_num, score_history, lines_history, args, filename):
    batch_size = 64
    alpha = 0.0003
    max_steps = args.steps
    plot_path = f'plots/{filename}'
    results_path = f'results/{filename}'
    lines_path = f'results/{filename}'
...
def run_DQN(args, filename):
    ...
    for process_num in range(threads):
            p = mproc.Process(target=run_worker, args=(process_num, score_history, lines_history))
            p.start()
            processes.append(p)
    for p in processes:
            p.join()
